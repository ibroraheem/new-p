...
const login = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Login successful", token: token, email: user.local.email, firstName: user.firstName, lastName: user.lastName, id:user._id, avatar:user.avatar})
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: error.message })
    }
